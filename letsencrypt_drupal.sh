...
# Example call with Dehydrated switches and option to skip self-update:
# ./letsencrypt_drupal.sh "projectname" "prod" "-f" -u
...
# Basic variables.
DEHYDRATED_SWITCHES="$3"
SCRIPT_OPTIONS="$4"

SCRIPT_OPTION_SKIP_SELFUPDATE="0"
if [[ $SCRIPT_OPTIONS =~ "^-.*u.*" ]]; then
  SCRIPT_OPTION_SKIP_SELFUPDATE="1"
fi
...
self_update() {
    if [ SCRIPT_OPTION_SKIP_SELFUPDATE == "1" ]; then
      return
    fi
